...
// 1.bulan
var ArrayBulan = array.filter(function (item) {
  return moment(item.created_at).format('MMMM') == 'February'
});
// console.log('February', ArrayBulan);
...
array.map((item) => {
  if (item.customer.name === "Annis") {
    item.items.map((sum) => {
      totalBelanja += (sum.qty * sum.price)
    })
  }
})

console.log('Total Belanja Ari = ' + 'Rp. '+ totalBelanja)

// 3 Under 300K
var ArrayHarga = []
ArrayHarga = array.filter(function (item) {
  let totall = null
  item.items.map((jumlah) => {
    totall += jumlah.qty * jumlah.price
  })
  return totall < 300000
});



// console.log('Under 300K', ArrayHarga);